...
파일을 가져오면 파일 이름에 타임스탬프가 추가됩니다(예: `myplan_May22_14:30:00.dxf`). 이를 통해 이름 충돌 없이 [Files](../files/)에 같은 도면의 여러 버전을 보관할 수 있습니다. 파일 이름에 이미 타임스탬프가 포함된 경우 그대로 사용됩니다.

도면은 가져오기 후 자동으로 브라우저 저장소(IndexedDB)에 저장되므로 [Files](../files/) 패널에 나타나고 페이지 새로고침 후에도 유지됩니다.
...
- [Files](../files/) — 브라우저에 저장된 도면 탐색 및 복원
