---
sidebar_position: 5
title: エクスポート — 図面を DXF または JSON でダウンロード（KulmanLab CAD）
...
**エクスポート**コマンドは現在の図面をファイルシステムにダウンロードします。2 つのフォーマットが利用できます: 他の CAD ツールとの互換性のための **DXF** と KulmanLab CAD 内での完全保存のための **JSON** です。
...
**DXF に含まれないもの:** 寸法エンティティと引出線。これらは標準 DXF では適切に表現できない KulmanLab 固有のデータ構造を使用しています。図面に注釈がある場合は、JSON を使用するか、視覚的に取得するために [印刷](../print/) を使用してください。
...
編集可能なファイルが必要な場合は**エクスポート**を使用してください。視覚的なスナップショットが必要な場合は [印刷](../print/) を使用してください。

## 関連コマンド

- [インポート](../import/) — DXF または JSON ファイルを開く
- [印刷](../print/) — キャンバスを PNG、JPEG、WebP、PDF 画像としてエクスポート
- [ファイル](../files/) — ブラウザストレージに保存された図面を参照
