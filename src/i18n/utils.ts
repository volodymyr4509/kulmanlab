...
export function localeUrl(lang: Lang, page: 'home' | 'faq' | 'changelog' | 'privacy' | 'about'): string {
  const suffix = page === 'home' ? '/' : `/${page}/`;
  if (lang === 'en') return suffix === '/' ? '/' : suffix;
  return `/${lang}${suffix}`;
}

export function buildAlternates(page: 'home' | 'faq' | 'changelog' | 'privacy' | 'about'): Array<{ hreflang: string; href: string }> {
